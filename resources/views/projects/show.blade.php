...
@section('content')
    ...
    <x-modal id="edit-project" class="p-10 lg:w-1/2 text-default">
        <edit-project-modal
            project='{"title":"{{ $project->title }}", "description":"{{ $project->description }}", "id":"{{ $project->id }}", "authId":{{ auth()->id() }}, "authIsOwner":"{{ $project->owner->id == auth()->id() }}", "members":{{ $project->members }}}'></edit-project-modal>
    </x-modal>
@endsection
